...
$f(\Omega_i)$ over the whole sky, as viewed from the rotation axis, at the
characteristic energies of 13~MeV and 34~MeV. These figures are analagous to
Fig.~\ref{fig:f_hot_ns}. I've sampled $f$ at these energies because the first is
...
\begin{figure}
  ...
  \end{subfigure}
  \caption[Disk emission as seen from rotation axis: at average energy]{
...
\begin{figure}
  ...
  \end{subfigure}
  \caption[Disk emission as seen from rotation axis: at high energy]{
...
\begin{enumerate}
  ...
  \item The $\nu_a$- and $\nu_x$-surfaces are sharp-edged at low energy,
    ...
    depth.
    This skews the ray tracing calculation of $f$ at energies very different
    ...
    \todo{why does this make $f$ sharp?}
  \item $f_{\nu_a}$ and $f_{\nu_x}$ appear similar at these two energies.
    ...
    to the similarity of these two $f$s. Even though the $\nu_x$-surface is
...
$f(\Omega_i)$ over the whole sky, as viewed from the equator, at the
...
\begin{figure}
  ...
  \end{subfigure}
  \caption[Disk emission as seen from equatorial plane: at average energy]{
    Neutrino distribution functions from the accretion disk over the sky in a
...
\begin{figure}
  ...
  \end{subfigure}
  \caption[Disk emission as seen from equatorial plane: at high energy]{
...
\begin{enumerate}
  \item The back side of the neutrinosurface behind the black hole, is visible
    when the front side of the neutrinosurface is not so large it geometrically
    obscures it.
    This due to the bending of rays by the spacetime curvature of the black
    hole. This has the effect of enlarging the emission surface copmared to
    that obtained by a simple Newtonian calculation. See
    Fig.~\ref{fig:schematic_ray_bending} for a schematic of this effect.
  \item The left side of the neutrinosurface (which is actually the right
    ...
    than those from the right side. See Fig.~\ref{fig:schematic_ray_doppler}
    for schematic.
\end{enumerate}
...
\begin{figure}
  ...
  \caption[Schematic of ray bending around the disk]{
    Cartoon reference of a meridional slice of the disk,
    explaining how the viewer on the right
    can see the far side of the neutrinosurface. The fluid is represented as
    a blue blob, and representative neutrino trajectories as dashed lines.
  }
...
\begin{figure}
  ...
  \caption[Schematic of doppler shift due to disk rotation]{
    Cartoon reference of an equatorial slice of the disk,
    explaining the doppler shift observed by the viewer on the
    right. The fluid velocity field is represented by arrows: longer is faster.
    The fluid at the bottom of the cartoon is moving roughly toward the viewer,
    boosting its neutrino emission; the opposite is true of the fluid at the top
    of the cartoon.
    The differential rotation of the disk makes this effect more pronounced
    for neutrinosurfaces deeper within the disk.
  }
...
invariants: $f$, $\diff^3p/p^t$, and $(p_{\nu \alpha} p_{\bar{\nu}}^\alpha)$.
...
Eqn.~\ref{eqn:qdot_general} is six-dimensional, and integrating it is a
...
$f_{\bar{\nu}}$, each requiring a separate geodesic integration taking
$\sym1$~s; on one processor this would take weeks of walltime.
Additionally, 10 points is a very low-resolution integration, and we wish
to repeat this calcultion at many points throughout the spatial volume.
...
\todo{cite}
\todo{explain why not parallelize}
...
$N^{1/2}$, as expected for a statistical estimate. This test required
...
\begin{figure}
  ...
  \caption[Convergence of $q_{\nu\bar{\nu}}$ in Homogeneous Minkowski Case]{
    ...
    At five different accuracy thresholds (the five clusters), I have computed
...
$\sym2000$ samples of the integrand.
...
\begin{figure}
  ...
  \caption[Convergence of $q_{\nu\bar{\nu}}$ in Asano-Fukuyama Sphere Case]{
    Convergence of the integration estimating $q_{\nu\bar{\nu}}$ in the
...
systematically underestimates its error.
In fact, the integration clearly converges to the incorrect value
underestimating $q_{\nu\bar{\nu}}$ by about 15\% at the highest resolution.
\todo{catalog sources for error in this case}
...
In cartesian coordinates, $\diff^3 x=\sqrt{-g}\, \diff x\,\diff y\,\diff z$.
...
of the Asano-Fukuyama test in Sec.~\ref{sssc:q_af_sphere}.
