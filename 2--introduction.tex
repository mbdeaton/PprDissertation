...
\end{equation}
With this parameterizations, Eqn.~\ref{eqn:f_boltzmann} becomes
\begin{equation}
  \frac{1}{c} \partial_t f + \frac{\diff}{\diff s} f =
  (hc)^2 \frac{\eta}{\varepsilon^2} - \chi f,
\end{equation}
...
\todo{write up $T_{\rm rad}$}
Eqn.~\ref{eqn:f_boltzmann} becomes
\begin{equation}
  \left(\frac{\diff}{\diff s} - \chi\right)f=0.
\end{equation}
\todo{derive optical depth integral, $\tau$}
...
include the projection of the momentum onto the observers worldline,
