...
\begin{itemize}
  ...
  \item Gamma ray burst: If an accretion disk forms---either after the
    ...
    \todo{Matt wants further explanation}
    compact binaries.
\end{itemize}
...
compact binary mergers. Unless astrophysicists are fundamentally
...
fluid is effectively viscous (which is likely, due to turbulence, magnetic
fields, or even radiative stresses),
\todo{cite each of these}
the gas in the inner disk gives up some of its angular momentum to gas a little
further out. The net effect is that the inner gas moves a little closer to the black hole,
decreasing its specific energy:
\begin{equation}
...
spirals in toward the black hole. But turbulence transfers it to smaller scales,
converting ordered kinetic energy into disordered thermal energy.
...
nuclei form and dissociate. At temperatures above $\sym0.5$~MeV, it is so small
...
The weak timescale, or how long it takes $Y_e$ to change due to the
charged-current reactions (Eqns.~\ref{eqn:beta_n_to_p} and~\ref{eqn:beta_p_to_n} and
...
$a^* \equiv J_{\rm BH}c/GM_{\rm BH}^2 =0.9$
(where $J_{\rm BH}$ is the black hole's spin angular momentum)
