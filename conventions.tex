...
\begin{table}
  ...
  \begin{tabular}{rll}
    ...
    $d\tau$               & spacetime interval        & $d\tau=(-dx^a dx^b \psi_{ab})^{1/2}$ \\
...
The interval between events separated by the vector $dx^\alpha$ is
computed with the spacetime metric, $\psi_{\alpha\beta}$, thus
\begin{equation}
  d\tau^2 = -\psi_{\alpha\beta} dx^\alpha dx^\beta.
\end{equation}
In everything that follows we employ the 3+1 splitting of the metric
...
\end{equation}
The decomposition of the metric and its inverse is
\begin{equation}
  \psi_{\mu\gamma} :=
  \left(
  \begin{matrix}
...
by the timelike congruence $n^\mu=dx^\mu/\alpha dt$, or component-wise,
